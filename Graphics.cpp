...
HRESULT Graphics::Initialize(unsigned int windowWidth, unsigned int windowHeight, HWND windowHandle, bool vsyncIfPossible)
{
	...
	// Create an upload heap for constant buffer data
	{
		...
		// This offseet changes as we use more CBs, and wraps around when full
...
void Graphics::ResizeBuffers(unsigned int width, unsigned int height)
{
	...
	// Whaat is the increment size between RTV descriptors in a 
...
// --------------------------------------------------------
// Copies the given data in the next "unused" spot in the 
// CBV upload heap (wrapping at the end, since we treat it
// like a ring buffer). Then creates a CBV in the next
...
{
	// How much space will we need? Each CBV must point to a
	// chunk of the upload heap that is a multiple of 256 bytes,
	// so we need to calculate and reserve that amount.
	...
	// Create a CBV for this section of the heap
	{
		...
		// Offset each based on how many descriptors we've used
		// - Note: cbvDescriptorOffset is a COUNT of descriptors,
		//   not bytes so we must calculate the size
		...
		// Describe the constant buffer view that points 
		// to our latest chunk of the CB upload heap
...
// --------------------------------------------------------
// Loads an image file and also creates a corresponding
// descriptor heap and descriptor. The heap is stored for
// later (as we'll need the descriptor to stay alive), 
// but will return the CPU descriptor handle so it can be
// used immediately for things (like materials).
// --------------------------------------------------------
D3D12_CPU_DESCRIPTOR_HANDLE Graphics::LoadTexture(
	const wchar_t* file, bool generateMips)
{
	...
	DirectX::CreateWICTextureFromFile(Device.Get(), upload, 
		file, texture.GetAddressOf(), generateMips);

	// Upload and wait for it to finish before returning texture
	// - Why auto here?
	auto finish = upload.End(CommandQueue.Get());
	finish.wait();

	// Now that we have the texture, add to our list and make
	// a CPU-side descriptor heap just for this texture's SRV.
	// Note that it would probably be better to put all texture
	// SRVs into teh same descriptor heap, but we don't know how
	// many we'll need until they're all loaded and this is a 
	...
	// - Using a null description results in the "default" SRV
...
// to the final descriptor heap that we use when drawing,
// and return the handle to the location of the first one.
// That handle is used to denote the start of a descriptor
// table, which is needed in the Material class.
// --------------------------------------------------------
...
{
	// Grab the actual heap start on both sides and 
	// offset to the next open SRV portion
	...
	// We know where to copy these descriptors,
	// so copy all of them and remember the new offset
...
// 
// size		 - How big shoudl the buffer be in bytes
// heapType	 - What kind of D3D12 heap? Default is D3D12_HEAP_TYPE_DEFAULT
// state	 - What state should hte resulting resource be in? 
//			   Default is D3D12_RESOURCE_STATE_COMMON
	...
	UINT64 alignment)
{
	...
	// Create the buffer
	Device->CreateCommittedResource(&heapDesc, D3D12_HEAP_FLAG_NONE, 
		&desc, state, 0, IID_PPV_ARGS(buffer.GetAddressOf()));
	return buffer;
}
...
{
	// Grab the actual heap start on both sides and 
	// offset to the next open SRV / UAV portion
	D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle = CBVSRVDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = CBVSRVDescriptorHeap->GetGPUDescriptorHandleForHeapStart();
	...
	// Update the overall offset if at the least one handle was reserved
